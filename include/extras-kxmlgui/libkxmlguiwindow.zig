...
pub const kxmlguiwindow = struct {
    ...
    pub fn OnMetaObject(self: ?*anyopaque, callback: *const fn () callconv(.c) QtC.QMetaObject) void {
        qtc.KXmlGuiWindow_OnMetaObject(@ptrCast(self), @intCast(@intFromPtr(callback)));
    }
    ...
    pub fn OnMetacast(self: ?*anyopaque, callback: *const fn (?*anyopaque, [*:0]const u8) callconv(.c) ?*anyopaque) void {
        qtc.KXmlGuiWindow_OnMetacast(@ptrCast(self), @intCast(@intFromPtr(callback)));
    }
    ...
    /// ` self: QtC.KXmlGuiWindow`
    ///
    /// ` callback: *const fn (self: QtC.KXmlGuiWindow, eventType: [*:0]u8, message: ?*anyopaque, result: *isize) callconv(.c) bool `
    ///
