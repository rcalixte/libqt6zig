...
pub const qgeoareamonitorinfo = struct {
    ...
    pub fn NotificationParameters(self: ?*anyopaque, allocator: std.mem.Allocator) map_constu8_qtcqvariant {
        ...
        while (i < _map.len) : (i += 1) {
            ...
            _ret.put(allocator, _entry_slice, _value) catch @panic("qgeoareamonitorinfo.NotificationParameters: Memory allocation failed");
        }
        return _ret;
    }
    ...
    pub fn SetNotificationParameters(self: ?*anyopaque, parameters: map_constu8_qtcqvariant, allocator: std.mem.Allocator) void {
        const parameters_keys = allocator.alloc(qtc.libqt_string, parameters.count()) catch @panic("qgeoareamonitorinfo.SetNotificationParameters: Memory allocation failed");
        defer allocator.free(parameters_keys);
        const parameters_values = allocator.alloc(QtC.QVariant, parameters.count()) catch @panic("qgeoareamonitorinfo.SetNotificationParameters: Memory allocation failed");
