...
pub const Attica__Field = extern struct {
    ...
    ///
    pub fn New(param1: anytype) Attica__Field {
        comptime _ = @TypeOf(param1)._is_Attica__Field;
        return .{ .ptr = qtc.Attica__Field_new(@ptrCast(param1.ptr)) };
    }
    ...
    ///
    pub fn Type(self: Attica__Field, allocator: std.mem.Allocator) []const u8 {
        var type_str = qtc.Attica__Field_Type(@ptrCast(self.ptr));
        defer qtc.libqt_string_free(&type_str);
        const type_ret = allocator.alloc(u8, type_str.len) catch @panic("attica__field.Type: Memory allocation failed");
    ...
    /// ` name: []const u8 `
    ///
    pub fn SetName(self: Attica__Field, name: []const u8) void {
    ...
    /// ` fieldsize: i32 `
    ///
    pub fn SetFieldsize(self: Attica__Field, fieldsize: i32) void {
        qtc.Attica__Field_SetFieldsize(@ptrCast(self.ptr), @bitCast(fieldsize));
    }
    ...
    /// ` required: bool `
    ///
    pub fn SetRequired(self: Attica__Field, required: bool) void {
        qtc.Attica__Field_SetRequired(@ptrCast(self.ptr), required);
    }
    ...
    pub fn Options(self: Attica__Field, allocator: std.mem.Allocator) []const []const u8 {
        ...
        for (0..options_arr.len) |i| {
            ...
            const options_buf = allocator.alloc(u8, options_data.len) catch @panic("attica__field.Options: Memory allocation failed");
    ...
    ///
    pub fn SetOptions(self: Attica__Field, allocator: std.mem.Allocator, options: []const []const u8) void {
        const options_arr = allocator.alloc(qtc.libqt_string, options.len) catch @panic("attica__field.SetOptions: Memory allocation failed");
...
pub const Attica__Publisher = extern struct {
    ...
    /// ` id: []const u8 `
    ///
    pub fn SetId(self: Attica__Publisher, id: []const u8) void {
    ...
    /// ` name: []const u8 `
    ///
    pub fn SetName(self: Attica__Publisher, name: []const u8) void {
        const name_str = qtc.libqt_string{
            .len = name.len,
            .data = name.ptr,
        };
    ...
    /// ` url: []const u8 `
    ///
    pub fn SetUrl(self: Attica__Publisher, url: []const u8) void {
