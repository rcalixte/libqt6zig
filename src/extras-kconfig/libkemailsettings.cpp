...
libqt_string KEMailSettings_Tr(const char* sourceText) {
    auto _ret = KEMailSettings::tr(sourceText);
...
libqt_string KEMailSettings_Tr2(const char* sourceText, const char* disambiguation) {
    auto _ret = KEMailSettings::tr(sourceText, disambiguation);
    // Convert QString from UTF-16 in C++ RAII memory to UTF-8 in manually-managed C memory
    QByteArray _b = _ret.toUtf8();
    libqt_string _str;
    _str.len = _b.length();
...
libqt_string KEMailSettings_Tr3(const char* sourceText, const char* disambiguation, int n) {
    auto _ret = KEMailSettings::tr(sourceText, disambiguation, static_cast<int>(n));
    // Convert QString from UTF-16 in C++ RAII memory to UTF-8 in manually-managed C memory
    QByteArray _b = _ret.toUtf8();
    libqt_string _str;
    _str.len = _b.length();
