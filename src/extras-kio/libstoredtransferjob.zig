...
pub const kio__storedtransferjob = struct {
    ...
    pub fn Metacall(self: ?*anyopaque, param1: i32, param2: i32, param3: *?*anyopaque) i32 {
        return qtc.KIO__StoredTransferJob_Metacall(@ptrCast(self), @intCast(param1), @intCast(param2), @ptrCast(param3));
    }
    ...
    pub fn AddMetaData2(self: ?*anyopaque, values: map_constu8_constu8, allocator: std.mem.Allocator) void {
        ...
        const values_values = allocator.alloc(qtc.libqt_string, values.count()) catch @panic("kio::storedtransferjob.AddMetaData2: Memory allocation failed");
    ...
    pub fn MergeMetaData(self: ?*anyopaque, values: map_constu8_constu8, allocator: std.mem.Allocator) void {
        ...
        defer allocator.free(values_keys);
        const values_values = allocator.alloc(qtc.libqt_string, values.count()) catch @panic("kio::storedtransferjob.MergeMetaData: Memory allocation failed");
        defer allocator.free(values_values);
        var i: usize = 0;
        var values_it = values.iterator();
        while (values_it.next()) |entry| : (i += 1) {
