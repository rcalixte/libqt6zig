...
pub const knscore__itemsmodel = struct {
    ...
    pub fn ItemData(self: ?*anyopaque, index: ?*anyopaque, allocator: std.mem.Allocator) map_i32_qtcqvariant {
        ...
        while (i < _map.len) : (i += 1) {
            ...
            _ret.put(allocator, _key, @ptrCast(_value)) catch @panic("knscore::itemsmodel.ItemData: Memory allocation failed");
        }
        return _ret;
    }
    ...
    pub fn RoleNames(self: ?*anyopaque, allocator: std.mem.Allocator) map_i32_u8 {
        ...
        while (i < _map.len) : (i += 1) {
            const _key = _keys[i];
            const _value = _values[i];
            const _value_slice = allocator.alloc(u8, _value.len) catch @panic("knscore::itemsmodel.RoleNames: Memory allocation failed");
            @memcpy(_value_slice, _value.data);
            _ret.put(allocator, _key, _value_slice) catch @panic("knscore::itemsmodel.RoleNames: Memory allocation failed");
        }
        return _ret;
    }
