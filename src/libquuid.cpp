...
QUuid* QUuid_new6(libqt_string string) {
    return new QUuid(QAnyStringView(string.data, string.len));
}
...
QUuid* QUuid_FromString(libqt_string string) {
    return new QUuid(QUuid::fromString(QAnyStringView(string.data, string.len)));
}
