...
pub const qsciapis = struct {
    ...
    pub fn QBaseMetacall(self: ?*anyopaque, param1: i32, param2: i32, param3: *?*anyopaque) i32 {
        return qtc.QsciAPIs_QBaseMetacall(@ptrCast(self), @bitCast(param1), @bitCast(param2), @ptrCast(param3));
    }
    ...
    pub fn QBaseCallTips(self: ?*anyopaque, context: []const []const u8, commas: i32, style: i32, shifts: []i32, allocator: std.mem.Allocator) []const []const u8 {
        ...
        const _arr: qtc.libqt_list = qtc.QsciAPIs_QBaseCallTips(@ptrCast(self), context_list, @bitCast(commas), @bitCast(style), shifts_list);
        var _str: [*]qtc.libqt_string = @ptrCast(@alignCast(_arr.data));
        defer {
            for (0.._arr.len) |i| {
                qtc.libqt_string_free(@ptrCast(&_str[i]));
