...
libqt_string KNetworkMounts_CanonicalSymlinkPath(KNetworkMounts* self, const libqt_string path) {
    QString path_QString = QString::fromUtf8(path.data, path.len);
    auto _ret = self->canonicalSymlinkPath(path_QString);
    // Convert QString from UTF-16 in C++ RAII memory to UTF-8 in manually-managed C memory
    QByteArray _b = _ret.toUtf8();
    libqt_string _str;
    _str.len = _b.length();
...
libqt_string KNetworkMounts_Tr2(const char* s, const char* c) {
    auto _ret = KNetworkMounts::tr(s, c);
    // Convert QString from UTF-16 in C++ RAII memory to UTF-8 in manually-managed C memory
    QByteArray _b = _ret.toUtf8();
    libqt_string _str;
    _str.len = _b.length();
...
libqt_string KNetworkMounts_Tr3(const char* s, const char* c, int n) {
    auto _ret = KNetworkMounts::tr(s, c, static_cast<int>(n));
    // Convert QString from UTF-16 in C++ RAII memory to UTF-8 in manually-managed C memory
    QByteArray _b = _ret.toUtf8();
    libqt_string _str;
    _str.len = _b.length();
