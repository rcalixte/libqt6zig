...
pub const qgeopositioninfosource = struct {
    ...
    pub fn CreateDefaultSource2(parameters: map_constu8_qtcqvariant, parent: ?*anyopaque, allocator: std.mem.Allocator) QtC.QGeoPositionInfoSource {
        const parameters_keys = allocator.alloc(qtc.libqt_string, parameters.count()) catch @panic("qgeopositioninfosource.CreateDefaultSource2: Memory allocation failed");
        defer allocator.free(parameters_keys);
        const parameters_values = allocator.alloc(QtC.QVariant, parameters.count()) catch @panic("qgeopositioninfosource.CreateDefaultSource2: Memory allocation failed");
    ...
    pub fn CreateSource2(sourceName: []const u8, parameters: map_constu8_qtcqvariant, parent: ?*anyopaque, allocator: std.mem.Allocator) QtC.QGeoPositionInfoSource {
        ...
        const parameters_keys = allocator.alloc(qtc.libqt_string, parameters.count()) catch @panic("qgeopositioninfosource.CreateSource2: Memory allocation failed");
        defer allocator.free(parameters_keys);
        const parameters_values = allocator.alloc(QtC.QVariant, parameters.count()) catch @panic("qgeopositioninfosource.CreateSource2: Memory allocation failed");
        defer allocator.free(parameters_values);
        var i: usize = 0;
        var parameters_it = parameters.iterator();
        while (parameters_it.next()) |entry| : (i += 1) {
...
pub const enums = struct {
    ...
    pub const PositioningMethod = enum {
        ...
        pub const NonSatellitePositioningMethods: i64 = 4294967040;
        pub const AllPositioningMethods: i64 = 4294967295;
    };
};
