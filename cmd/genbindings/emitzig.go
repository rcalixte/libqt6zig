...
func (p CppParameter) needsPointer(paramType string) bool {
	return IsKnownClass(p.ParameterType) &&
		!(strings.Contains(paramType, "anytype") || strings.Contains(paramType, "[]"))
}
...
func mapParamToString(param string) string {
	...
	maybeSlice := ""
	if (strings.Count(param, "[]") > 1 && !strings.Contains(param, "const")) || (strings.HasPrefix(param, "[]") && !strings.Contains(param, "u8")) {
		maybeSlice = "Slice"
	}
	...
	return maybeSlice + string(result)
}
...
func (p CppParameter) RenderTypeZig(zfs *zigFileState, isReturnType, fullEnumName bool) string {
	...
	if t1, t2, containerType, ok := p.QMapOf(); ok {
		...
		v = ifv(v == "SignOn__MechanismsList", "[]const []const u8", v)

		if IsKnownClass(t2Type) && !slices.Contains(zfs.currentClasses, t2.ParameterType) {
			zfs.imports["class:"+cabiClassName(t2Type)] = struct{}{}
			zfs.currentClasses = append(zfs.currentClasses, t2.ParameterType)
		}
	...
	if p.ByRef || p.Pointer || IsKnownClass(p.ParameterType) {
		if isReturnType {
			if p.needsPointer(ret) {
				ret = ifv(p.PointerCount > 1, "*", "") + ret
			}
		} else if p.QtClassType() {
			if !slices.Contains(zfs.currentClasses, p.ParameterType) {
				zfs.imports["class:"+cabiClassName(p.ParameterType)] = struct{}{}
				zfs.currentClasses = append(zfs.currentClasses, p.ParameterType)
			}
...
func (p CppParameter) renderReturnTypeZig(zfs *zigFileState, isSlot bool) (string, string, string) {
	...
	if IsKnownClass(ret) && !slices.Contains(zfs.currentClasses, p.ParameterType) {
		zfs.imports["class:"+cabiClassName(ret)] = struct{}{}
		zfs.currentClasses = append(zfs.currentClasses, p.ParameterType)
	}

	if p.PointerCount > 1 && IsKnownClass(p.ParameterType) {
		return strings.Repeat("*", p.PointerCount-1) + "QtC." + p.ParameterType, "", ""
	}
...
func (zfs *zigFileState) emitCommentParametersZig(params []CppParameter, isSlot bool) string {
	...
	for _, p := range params {
		...
		if IsKnownClass(paramType) && p.PointerCount > 1 {
			if !slices.Contains(zfs.currentClasses, p.ParameterType) {
				zfs.imports["class:"+cabiClassName(paramType)] = struct{}{}
				zfs.currentClasses = append(zfs.currentClasses, p.ParameterType)
			}

			paramType = "*" + paramType
		}
		if p.needsPointer(paramType) && p.PointerCount > 1 {
			paramType += ".ptr"
		}
		if p.IntType() && (p.Pointer || p.ByRef) {
			if !p.GlIntType() {
		...
		if isSlot {
			...
			} else if p.PointerCount <= 1 && IsKnownClass(p.ParameterType) && p.ParameterType != "QByteArrayView" {
				paramType = cabiClassName(p.ParameterType)
			} else {
				// C calling convention limitations
				switch paramType {
		...
		if _, ok := zfs.currentMethods[paramName]; !isSlot && (ok || zigReservedWord(paramName)) {
			paramName = "_" + paramName
		}

		tmp = append(tmp, paramName+": "+paramType)
	}
...
func (zfs *zigFileState) emitParametersZig(params []CppParameter, isSlot bool) string {
	...
	for _, p := range params {
		...
		if _, ok := zfs.currentMethods[param]; ok || zigReservedWord(param) {
			param = "_" + param
		}
		if p.needsPointer(paramType) {
			paramType = ifv(p.PointerCount > 2, "*", "") + paramType
		}
		if (p.ParameterType == "GLvoid" || p.ParameterType == "void") && (p.Pointer || p.ByRef) {
			paramType = ifv(p.PointerCount > 1, "*", "") + "?*" + ifv(p.Const, "const ", "") + "anyopaque"
		}
		...
		if isSlot {
			if t, _, ok := p.QListOf(); ok && (t.ParameterType != "QString" && t.ParameterType != "QByteArray") {
				...
				pType := cabiClassName(p.ParameterType)
				if !slices.Contains(zfs.currentClasses, p.ParameterType) {
					zfs.imports["class:"+pType] = struct{}{}
					zfs.currentClasses = append(zfs.currentClasses, p.ParameterType)
				}
...
func (zfs *zigFileState) emitParameterZig2CABIForwarding(p CppParameter) (preamble, rvalue string) {
	...
	if _, ok := zfs.currentMethods[p.ParameterName]; ok || zigReservedWord(p.ParameterName) {
	...
	if p.ParameterType == "QString" || p.ParameterType == "QByteArray" || p.ParameterType == "QByteArrayView" ||
		...
		if t.ParameterType == "QString" || t.ParameterType == "QByteArray" || t.ParameterType == "SignOn::MethodName" ||
			...
			preamble += "const " + nameprefix + "_arr = allocator.alloc(qtc.libqt_string, " + p.ParameterName + `.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
			preamble += "defer allocator.free(" + nameprefix + "_arr);\n"
			preamble += "for (" + p.ParameterName + ", 0.." + p.ParameterName + ".len) |str_item, i|\n"
			preamble += "    " + nameprefix + "_arr[i] = .{\n"
			preamble += "        .len = str_item.len,\n"
			preamble += "        .data = str_item.ptr,\n"
			preamble += "    };\n"

		} else if t.ParameterType == "char" && t.PointerCount == 1 {
			preamble += "const " + nameprefix + "_cStr = allocator.alloc([*:0]" + ifv(t.Const, "const ", "") + " u8, " + p.ParameterName + `.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
			preamble += "defer allocator.free(" + nameprefix + "_cStr);\n"
			preamble += "for (" + p.ParameterName + ", 0.." + p.ParameterName + ".len) |" + nameprefix + "_item, i|\n"
			preamble += "    " + nameprefix + "_cStr[i] = @ptrCast(" + nameprefix + "_item.ptr);"

		} else if l, _, ok := t.QListOf(); ok {
			if IsKnownClass(l.ParameterType) && !slices.Contains(zfs.currentClasses, l.ParameterType) {
				zfs.imports["class:"+cabiClassName(l.ParameterType)] = struct{}{}
				zfs.currentClasses = append(zfs.currentClasses, l.ParameterType)
			}
			...
			if (f.ParameterType == "QString" || f.ParameterType == "QByteArray") && (s.ParameterType == "QString" || s.ParameterType == "QByteArray") {
				preamble += "const " + nameprefix + "_pairs = allocator.alloc(qtc.libqt_pair, " + p.ParameterName + `.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
				...
				preamble += "    };\n"
				preamble += "    " + nameprefix + "_str[i * 2 + 1] = qtc.libqt_string{\n"
				preamble += "        .len = " + nameprefix + "_item.second.len,\n"
				preamble += "        .data = " + nameprefix + "_item.second.ptr,\n"
				preamble += "    };\n"
				preamble += "    " + nameprefix + "_pairs[i] = qtc.libqt_pair{\n"
				preamble += "        .first = @ptrCast(&" + nameprefix + "_str[i * 2]),\n"
				preamble += "        .second = @ptrCast(&" + nameprefix + "_str[i * 2 + 1]),\n"
				preamble += "    };\n"
				preamble += "}"
			}
		}
		...
		if t.ParameterType == "QString" || t.ParameterType == "QByteArray" || t.ParameterType == "SignOn::MethodName" ||
			...
			preamble += "    .data = @ptrCast(" + nameprefix + "_cStr),\n"

		} else if t.QtClassType() {
			if !slices.Contains(zfs.currentClasses, t.ParameterType) {
				zfs.imports["class:"+cabiClassName(t.ParameterType)] = struct{}{}
				zfs.currentClasses = append(zfs.currentClasses, t.ParameterType)
			}
			...
			if (f.ParameterType == "QString" || f.ParameterType == "QByteArray") && (s.ParameterType == "QString" || s.ParameterType == "QByteArray") {
				...
				if IsKnownClass(f.ParameterType) && !slices.Contains(zfs.currentClasses, f.ParameterType) {
					zfs.imports["class:"+cabiClassName(f.ParameterType)] = struct{}{}
					zfs.currentClasses = append(zfs.currentClasses, f.ParameterType)
				}
				if IsKnownClass(s.ParameterType) && !slices.Contains(zfs.currentClasses, s.ParameterType) {
					zfs.imports["class:"+cabiClassName(s.ParameterType)] = struct{}{}
					zfs.currentClasses = append(zfs.currentClasses, s.ParameterType)
				}
		...
		if t.ParameterType == "QString" || t.ParameterType == "QByteArray" {
			preamble += "const " + nameprefix + "_arr = allocator.alloc(qtc.libqt_string, " + nameprefix + `_count) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
			preamble += "defer allocator.free(" + nameprefix + "_arr);\n"
			preamble += "var " + nameprefix + "_it = " + p.ParameterName + ".keyIterator();\n"
			...
			preamble += "const " + nameprefix + "_keys = allocator.alloc(" + e.EnumTypeZig + ", " + nameprefix + `_count) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
		...
		if IsKnownClass(vTypeZig) && !slices.Contains(zfs.currentClasses, vType.ParameterType) {
			zfs.imports["class:"+cabiClassName(vTypeZig)] = struct{}{}
			zfs.currentClasses = append(zfs.currentClasses, vType.ParameterType)
		}
		zfs.imports[hashMapType+vParam] = struct{}{}

		if !isQMulti {
			if strings.HasPrefix(vParam, "[]") && IsKnownClass(vParam[2:]) {
		...
		if innerType, _, ok := vType.QListOf(); ok {
			if innerType.ParameterType == "QString" || innerType.ParameterType == "QByteArray" {
				...
				if !slices.Contains(zfs.currentClasses, innerType.ParameterType) {
					zfs.imports["class:"+cabiClassName(innerType.ParameterType)] = struct{}{}
					zfs.currentClasses = append(zfs.currentClasses, innerType.ParameterType)
				}
		...
		var maybeKeyPointer, maybeValuePointer string
		...
		preamble += "const " + nameprefix + "_count = " + p.ParameterName + ".count();\n"
		preamble += "const " + nameprefix + "_keys = allocator.alloc(" + maybeKeyPointer + kTypeZig + ", " + nameprefix + `_count) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
		preamble += "defer allocator.free(" + nameprefix + "_keys);\n"
		preamble += "const " + nameprefix + "_values = allocator.alloc(" + maybeValuePointer + vAllocType + ", " + nameprefix + `_count) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
		preamble += "defer allocator.free(" + nameprefix + "_values);\n"

		if isQMulti {
			preamble += "const " + nameprefix + "_inners = allocator.alloc([]" + valueParamType + ", " + nameprefix + `_count) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
			preamble += "defer {\n"
		...
		if (fType.IntType() || IsKnownClass(fType.ParameterType)) && (sType.IntType() || IsKnownClass(sType.ParameterType)) {
			firstValue := ifv(IsKnownClass(fType.ParameterType), "@ptrCast("+p.ParameterName+".first.ptr)", "@bitCast("+p.ParameterName+".first)")
			secondValue := ifv(IsKnownClass(sType.ParameterType), "@ptrCast("+p.ParameterName+".second.ptr)", "@bitCast("+p.ParameterName+".second)")
		...
		case 2:
			...
			// Q*Application constructor
			// Qt has a warning in the docs saying these pointers must be valid for
			...
			rvalue = "@ptrCast(" + nameprefix + "_chararr)"
		...
		// We want our functions to accept the Zig wrapper type, and forward with the pointer
		if !slices.Contains(zfs.currentClasses, p.ParameterType) {
			zfs.imports["class:"+cabiClassName(p.ParameterType)] = struct{}{}
			zfs.currentClasses = append(zfs.currentClasses, p.ParameterType)
		}
...
func (zfs *zigFileState) emitCabiToZig(assignExpr string, rt CppParameter, rvalue string) string {
	...
	if rt.Void() {
		...
		if IsKnownClass(t.ParameterType) || IsKnownTypeDef(t.ParameterType) || t.IntType() ||
			...
			} else if IsKnownClass(t.ParameterType) && !slices.Contains(zfs.currentClasses, t.ParameterType) {
				zfs.imports["class:"+cabiClassName(t.ParameterType)] = struct{}{}
				zfs.currentClasses = append(zfs.currentClasses, t.ParameterType)
			}

			afterword += "defer qtc.libqt_free(" + namePrefix + "_arr.data);\n"
			afterword += "const " + namePrefix + "_ret = allocator.alloc(" + arrType + ", " + namePrefix + `_arr.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
			afterword += "const " + namePrefix + "_data_val: [*]" + arrCast + arrType + " = @ptrCast(@alignCast(" + namePrefix + "_arr.data));\n"
			if IsKnownClass(t.ParameterType) {
				afterword += "for (0.." + namePrefix + "_arr.len) |j|\n"
				afterword += "    " + namePrefix + "_ret[j] = .{ .ptr = " + namePrefix + "_data_val[j] };\n"
			} else {
				afterword += "@memcpy(" + namePrefix + "_ret, " + namePrefix + "_data_val[0.." + namePrefix + "_arr.len]);\n"
			}
			...
			afterword += "        const " + namePrefix + `_buf = allocator.allocSentinel(u8, cstr_len, 0) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Buffer allocation failed");` + "\n"
			...
			afterword += "const " + namePrefix + "_ret = allocator.alloc(" + arrType + ", " + namePrefix + `_arr.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
			afterword += "for (0.." + namePrefix + "_arr.len) |i| {\n"
			afterword += "    const " + namePrefix + "_data_val = " + namePrefix + "_str[i];\n"
			afterword += "    const " + namePrefix + "_buf = allocator.alloc(u8, " + namePrefix + `_data_val.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
			afterword += "    @memcpy(" + namePrefix + "_buf, " + namePrefix + "_data_val.data[0.." + namePrefix + "_data_val.len]);\n"
			afterword += "    " + namePrefix + "_ret[i] = " + namePrefix + "_buf;\n"
			afterword += "}\n"
			...
			if (f.IntType() || IsKnownClass(f.ParameterType)) && (s.IntType() || IsKnownClass(s.ParameterType)) {
				...
				afterword += "const " + namePrefix + "_data_val: [*]" + arrType + " = @ptrCast(@alignCast(" + namePrefix + "_arr.data));\n"
				afterword += "defer qtc.libqt_free(" + namePrefix + "_arr.data);\n"
				afterword += "const " + namePrefix + "_ret = allocator.alloc(" + arrType + ", " + namePrefix + `_arr.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
				afterword += "@memcpy(" + namePrefix + "_ret, " + namePrefix + "_data_val[0.." + namePrefix + "_arr.len]);\n"

			} else {
				afterword += "const " + namePrefix + "_data_val: [*]qtc.libqt_pair = @ptrCast(@alignCast(" + namePrefix + "_arr.data));\n"
				...
				if f.ParameterType == "QString" || f.ParameterType == "QByteArray" {
					firstFree = "        qtc.libqt_string_free(@ptrCast(@alignCast(" + namePrefix + "_data_val[i].first)));\n"
					firstLoop = "    const " + namePrefix + "_first_str: *qtc.libqt_string = @ptrCast(@alignCast(" + namePrefix + "_data_val[i].first));\n"
					firstLoop += "    const " + namePrefix + "_first_slice = allocator.alloc(u8, " + namePrefix + `_first_str.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
					...
					if !slices.Contains(zfs.currentClasses, f.ParameterType) {
						zfs.imports["class:"+cabiClassName(f.ParameterType)] = struct{}{}
						zfs.currentClasses = append(zfs.currentClasses, f.ParameterType)
					}

					firstParam = ".{ .ptr = @ptrCast(" + namePrefix + "_data_val[i].first) }"
				} else if f.IntType() {
					firstFree = "        qtc.libqt_free(@ptrCast(@alignCast(" + namePrefix + "_data_val[i].first)));\n"
					firstParam = "@as(*" + f.parameterTypeZig() + ", @ptrCast(@alignCast(" + namePrefix + "_data_val[i].first))).*"
				} else {
				...
				if s.ParameterType == "QString" || s.ParameterType == "QByteArray" {
					secondFree = "        qtc.libqt_string_free(@ptrCast(@alignCast(" + namePrefix + "_data_val[i].second)));\n"
					secondLoop = "    const " + namePrefix + "_second_str: *qtc.libqt_string = @ptrCast(@alignCast(" + namePrefix + "_data_val[i].second));\n"
					secondLoop += "    const " + namePrefix + "_second_slice = allocator.alloc(u8, " + namePrefix + `_second_str.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed")` + ";\n"
					...
					if !slices.Contains(zfs.currentClasses, s.ParameterType) {
						zfs.imports["class:"+cabiClassName(s.ParameterType)] = struct{}{}
						zfs.currentClasses = append(zfs.currentClasses, s.ParameterType)
					}

					secondParam = ".{ .ptr = @ptrCast(" + namePrefix + "_data_val[i].second) }"
				} else if s.IntType() {
					secondFree = "        qtc.libqt_free(@ptrCast(@alignCast(" + namePrefix + "_data_val[i].second)));\n"
					secondParam = "@as(*" + s.parameterTypeZig() + ", @ptrCast(@alignCast(" + namePrefix + "_data_val[i].second))).*"
				} else {
			...
			if p.ParameterType == "QString" || p.ParameterType == "QByteArray" {
				...
				afterword += "for (0.." + namePrefix + "_arr.len) |i| {\n"
				afterword += "    const " + namePrefix + "_data_val = " + namePrefix + "_str[i];\n"
				afterword += "    const " + namePrefix + "_strlist = allocator.alloc(" + arrAllocType + ", " + namePrefix + `_data_val.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
				afterword += "    var " + namePrefix + "_strdata: [*]qtc.libqt_string = @ptrCast(@alignCast(" + namePrefix + "_data_val.data));\n"
				afterword += "    for (0.." + namePrefix + "_data_val.len) |j| {\n"
				afterword += "        const " + namePrefix + "_buf = allocator.alloc(u8, " + namePrefix + `_strdata[j].len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
				...
				afterword += "const " + namePrefix + "_ret = allocator.alloc(" + arrType + ", " + namePrefix + `_arr.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
				afterword += "for (0.." + namePrefix + "_arr.len) |i| {\n"
				afterword += "    const " + namePrefix + "_data_val: [*]QtC." + p.ParameterType + " = @ptrCast(@alignCast(" + namePrefix + "_list[i].data));\n"
				afterword += "    " + namePrefix + "_ret[i] = allocator.alloc(" + p.ParameterType + ", " + namePrefix + `_list[i].len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
				afterword += "    for (0.." + namePrefix + "_list[i].len) |j|\n"
				afterword += "        " + namePrefix + "_ret[i][j] = .{ .ptr = _data_val[j] };\n"
				afterword += "}\n"
				...
				afterword += "const " + namePrefix + "_ret = allocator.alloc([]" + arrType + ", " + namePrefix + `_arr.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
				afterword += "for (0.." + namePrefix + "_arr.len) |i| {\n"
				afterword += "    const " + namePrefix + "_data_val: [*]" + arrType + " = @ptrCast(@alignCast(" + namePrefix + "_list[i].data));\n"
				afterword += "    " + namePrefix + "_ret[i] = allocator.alloc(" + arrType + ", " + namePrefix + `_list[i].len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
				afterword += "    for (0.." + namePrefix + "_list[i].len) |j|\n"
				afterword += "        " + namePrefix + "_ret[i][j] = _data_val[j];\n"
				afterword += "}\n"
		...
		if t.ParameterType == "QString" || t.ParameterType == "QByteArray" {
			...
			zfs.imports["std"] = struct{}{}
			var setType, setValue, maybeDecl, maybeDeclClose string
			...
			if t.IsKnownEnum() {
				...
				setValue = "QtC." + t.RenderTypeZig(zfs, true, true)
				maybeDecl = ".{ .ptr = "
				maybeDeclClose = " }"
			}
			...
			afterword += namePrefix + "_ret.ensureTotalCapacity(allocator, @intCast(" + namePrefix + `_set.len)) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Total capacity allocation failed");` + "\n"
			afterword += "const " + namePrefix + "_data_val: [*]" + setValue + " = @ptrCast(@alignCast(" + namePrefix + "_set.data));\n"
			afterword += "for (0.." + namePrefix + "_set.len) |i|\n"
			afterword += "    " + namePrefix + "_ret.putAssumeCapacity(" + maybeDecl + namePrefix + "_data_val[i]" + maybeDeclClose + ", {});\n"
		...
		if !strings.HasPrefix(valParam, "qtc.lib") && IsKnownClass(valParam) || (strings.HasPrefix(valParam, "Slice") && IsKnownClass(valParam[5:])) {
			valParam = vType.RenderTypeZig(zfs, true, true)
		}

		if strings.HasPrefix(valParam, "[]") && IsKnownClass(valParam[2:]) {
			listType = valParam[2:]
		...
		var maybeKPointer, maybeKDecl, maybeKDeclClose, maybeVPointer, maybeVDecl, maybeVDeclClose string
		if IsKnownClass(keyType) {
			maybeKPointer = "QtC."
			maybeKDecl = ".{ .ptr = "
			maybeKDeclClose = " }"
		}
		if !isQMulti && IsKnownClass(vType.ParameterType) {
			maybeVPointer = "QtC."
			maybeVDecl = ".{ .ptr = "
			maybeVDeclClose = " }"
		}
		...
		afterword += namePrefix + "_ret.ensureTotalCapacity(allocator, @intCast(" + namePrefix + `_map.len)) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Total capacity allocation failed");` + "\n"
		...
		if isQMulti {
			...
			afterword += "const " + namePrefix + "_values: [*]" + maybeVPointer + valParam + " = @ptrCast(@alignCast(" + namePrefix + "_map.values));\n"
			deferVal = "qtc.libqt_free(" + namePrefix + "_values[i].data);\n"
		}
		...
		afterword += "const " + namePrefix + "_keys: [*]" + maybeKPointer + keyType + " = @ptrCast(@alignCast(" + namePrefix + "_map.keys));\n"
		afterword += "const " + namePrefix + "_values: [*]" + maybeVPointer + valParam + " = @ptrCast(@alignCast(" + namePrefix + "_map.values));\n"
		...
		if stringKey {
			afterword += "const " + namePrefix + "_entry_slice = allocator.alloc(u8, " + namePrefix + `_key.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
		...
		if isQMulti {
			if vType.ParameterType == "QByteArray" || vType.ParameterType == "QString" {
				...
				afterword += "const " + namePrefix + "_value_slice = allocator.alloc(" + vType.RenderTypeZig(zfs, true, true) + ", " + namePrefix + `_value.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
				afterword += "for (0.." + namePrefix + "_value.len) |j| {\n"
				afterword += "    const " + namePrefix + "_vslice = allocator.alloc(u8, " + namePrefix + `_value_strings[j].len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
				...
				afterword += "const " + namePrefix + "_value_slice = allocator.alloc(" + valueAllocType + ", " + namePrefix + `_value.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
				afterword += "const " + namePrefix + "_value_data: [*]QtC." + valueAllocType + " = @ptrCast(@alignCast(" + namePrefix + "_value.data));\n"
				afterword += "for (0.." + namePrefix + "_value.len) |j|\n"
				afterword += "    " + namePrefix + "_value_slice[j] = .{ .ptr = " + namePrefix + "_value_data[j] };\n"
			...
			afterword += "const " + namePrefix + "_value_slice = allocator.alloc(u8, " + namePrefix + `_value.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
			afterword += "@memcpy(" + namePrefix + "_value_slice, " + namePrefix + "_value.data);\n"
			retVal = namePrefix + "_value_slice"
		} else if listValue {
			maybePtr := ifv(IsKnownClass(listType), "QtC.", "")
			afterword += "const " + namePrefix + "_value_slice = allocator.alloc(" + listType + ", " + namePrefix + `_value.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
			afterword += "const " + namePrefix + "_value_data: [*]" + maybePtr + listType + " = @ptrCast(@alignCast(" + namePrefix + "_value.data));\n"
			if maybePtr == "" {
		...
		if (kType.IntType() || IsKnownClass(kType.ParameterType)) && (vType.IntType() || IsKnownClass(vType.ParameterType)) {
			...
			if kType.ParameterType == "QString" {
				...
				afterword += "}\n"
				afterword += "const " + namePrefix + "_first_slice = allocator.alloc(u8, " + namePrefix + `_first_str.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
			...
			if vType.ParameterType == "QString" {
				...
				afterword += "}\n"
				afterword += "const " + namePrefix + "_second_slice = allocator.alloc(u8, " + namePrefix + `_second_str.len) catch @panic("` + zfs.currentClassName + "." + zfs.currentMethodName + `: Memory allocation failed");` + "\n"
		...
		if !slices.Contains(zfs.currentClasses, rt.ParameterType) {
			zfs.imports["class:"+cabiClassName(rt.ParameterType)] = struct{}{}
			zfs.currentClasses = append(zfs.currentClasses, rt.ParameterType)
		}
		...
		afterword += `if (ret_str == null) return "";`
...
func collectInheritedMethodsForZig(class string, seenMethods map[string]struct{}, zfs *zigFileState) []InheritedMethod {
	...
	if pkg, ok := KnownClassnames[class]; ok {
		...
		for _, parentClass := range pkg.Class.DirectInherits {
			...
			if !slices.Contains(zfs.parentClasses, cClassName) {
				zfs.parentClasses = append(zfs.parentClasses, cClassName)
			}
...
const qtc = @import("qt6c");`)
	...
	ret.WriteString("%%_IMPORTLIBS_%% %%_STRUCTDEFS_%%\n")
	...
	for _, c := range src.Classes {
		...
		zfs.parentClasses = []string{zigStructName}
		...
		for _, base := range c.DirectInherits {
			cBaseName := cabiClassName(base)
			zfs.parentClasses = append(zfs.parentClasses, cBaseName)
		...
		footerNeeded := false
		if len(c.Ctors) > 0 || len(c.Methods) > 0 || len(virtualMethods) > 0 || c.ClassName == "Poppler::AnnotationAppearance" ||
			...
			if AllowStructDef(c.ClassName) && !isBindingRemoved(c.ClassName) {
				...
				for _, pc := range zfs.parentClasses {
					if !strings.Contains(pc, "<") {
						ret.WriteString("pub const _is_" + pc + " = {};\n")
					}
				}
			}
		}
		...
		for _, s := range [][]CppMethod{baseMethods, virtualMethods, privateSignals} {
			for _, m := range s {
				zfs.currentMethods[m.MethodName] = struct{}{}
			}
		}
...
`)
		...
		if c.HasTrivialCopyAssign {
			...
			ret.WriteString("/// Shallow copy `other` into `self` in C++ memory\n///\n" +
				"/// ## Parameters:\n///\n/// ` self: " + zigStructName + " `\n///\n/// ` other: " + zigStructName + " `\n///\n" +
				"pub fn copyAssign(self: " + zigStructName + ", other: " + zigStructName + ") void {\n" +
				"qtc." + zigStructName + "_CopyAssign(@ptrCast(self.ptr), @ptrCast(other.ptr));\n}\n\n")
		}
		...
		if c.HasTrivialMoveAssign {
			...
			ret.WriteString("/// Move `other` into `self` and invalidate `other` in C++ memory\n///\n" +
				"/// ## Parameters:\n///\n/// ` self: " + zigStructName + " `\n///\n/// ` other: " + zigStructName + " `\n///\n" +
				"pub fn moveAssign(self: " + zigStructName + ", other: " + zigStructName + ") void {\n" +
...
if (builtin.target.os.tag != .macos) @compileError("Unsupported operating system");`
			...
			isReserved := zigReservedWord(m.MethodName)
			methodName := ifv(isReserved, m.MethodName+"0", m.safeMethodName())
			...
			if isReserved && m.MethodName != "self" {
				ret.WriteString("\n    pub const " + `@"` + m.MethodName + `" = ` + methodName + ";\n")
			}
			...
			if allocatorParam != "" {
				selfParam += "\n///\n/// ` " + allocatorParam + " `"
				allocatorParam += maybeComma
			}

			fnMethod := methodName + "(self: " + zigStructName + ifv(allocComma != "" || maybeComma != "", ", ", "")
			if m.IsStatic && !m.IsProtected {
				selfParam = ifv(allocatorParam != "", "\n///\n/// ` allocator: std.mem.Allocator `", "")
				fnMethod = methodName + "("
			}
...
`)
		...
		for _, m := range privateSignals {
			...
			zfs.currentMethodName = m.safeMethodName()
			...
			ret.WriteString(inheritedFrom + docCommentUrl + headerComment + "/// ` self: " + zigStructName + " `\n///\n/// ` callback: *const fn (self: " +
				zigStructName + maybeComma + zfs.emitCommentParametersZig(m.Parameters, true) + ") callconv(.c) void `\n///\n" +
				"    pub fn on" + mSafeMethodName + "(self: " + zigStructName + ", callback: *const fn (" + zigStructName +
				maybeComma + zfs.emitParametersZig(m.Parameters, true) + ") callconv(.c) void) void {\n" +
				"qtc." + cmdStructName + "_Connect_" + cSafeMethodName + "(@ptrCast(self.ptr), @bitCast(@intFromPtr(callback)));\n}\n")
		...
		if c.CanDelete && !isBindingRemoved(zigStructName) && (len(c.Methods) > 0 || len(c.VirtualMethods()) > 0 || len(c.Ctors) > 0) {
			...
			ret.WriteString(ifv(pageUrl != "", pageUrl+"\n///\n", "\n") +
				...
				"/// ## Parameter:\n///\n" +
				"/// ` self: " + zigStructName + " `\n///\n" +
				"    pub fn delete(self: " + zigStructName + ") void {\n" +
				"qtc." + zigStructName + "_Delete(@ptrCast(self.ptr));\n}")
		}
		if footerNeeded {
			ret.WriteString("\n};\n")
		}
	...
	if len(zfs.imports) > 0 {
		...
		for k := range zfs.imports {
			if k == "std" {
				...
				allImports = append(allImports, "const "+k[6:]+` = @import("libqt6").`+k[6:]+";")
			} else {
				if strings.Contains(k, ",") && strings.Count(k, ",") >= 2 {
					...
					case "StringArrayHashMap", "StringHashMap":
						maybeArray := mapType == "StringArrayHashMap"
						key := ifv(maybeArray, "Array", "") + "Map_" + keyType + "_" + mapParamToString(valueType)
						value := "std." + ifv(maybeArray, "array_hash_map.String", mapType+"Unmanaged") + "(" + valueType + ")"
						structDef = append(structDef, "const "+key+" = "+value+";")
						zigTypes[key] = value
					case "AutoArrayHashMap", "AutoHashMap":
						autoKeyType := keyType
						keyType = mapParamToString(keyType)
						maybeArray := mapType == "AutoArrayHashMap"
						key := ifv(maybeArray, "Array", "") + "Map_" + keyType + "_" + mapParamToString(valueType)
						value := "std." + ifv(maybeArray, "array_hash_map.Auto", mapType+"Unmanaged") + "(" + autoKeyType + ", " + valueType + ")"
						structDef = append(structDef, "const "+key+" = "+value+";")
						zigTypes[key] = value
					}
					...
					keyType := kSplit[1]
					valueType := kSplit[2]
					if mapParamToString(keyType) != "anytype" {
